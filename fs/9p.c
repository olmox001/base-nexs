...
static P9Fid *fid_find(uint32_t fid) {
  for (int i = 0; i < P9_MAX_FIDS; i++)
    if (s_fids[i].in_use && s_fids[i].fid == fid)
      return &s_fids[i];
  return NULL;
}
...
int p9_server_handle(const uint8_t *req, size_t req_len, uint8_t *resp,
                     size_t resp_max) {
  if (req_len < 7)
    return -1;
  ...
  case P9_TVERSION: {
    ...
    if (client_msize < s_msize)
      s_msize = client_msize;
  ...
  case P9_TATTACH: {
    ...
    uint16_t uname_len = read_u16(p);
    p += 2 + uname_len;
    uint16_t aname_len = read_u16(p);
    p += 2 + aname_len;
    (void)p;
