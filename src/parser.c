...
static ASTNode *parse_block(Parser *p) {
  ...
  while (p->cur.kind != TK_RBRACE && p->cur.kind != TK_EOF && !p->had_error) {
    ...
    if (s) {
      ...
      if (!p->had_error && p->cur.kind != TK_NEWLINE && p->cur.kind != TK_RBRACE && p->cur.kind != TK_EOF) {
        snprintf(p->error_msg, sizeof(p->error_msg), 
                 "Riga %d: atteso fine riga o '}' dopo l'istruzione, trovato '%s'", p->cur.line, p->cur.text);
        p->had_error = 1;
        break;
      }
    }
