/*
 * lexer.c — NEXS Lexer (Tokenizer)
 * ==================================
 * Converte sorgente testuale in stream di Token.
 *
 * BUG FIX: "/" da solo è riconosciuto come TK_REGPATH (percorso radice)
 *          quando segue keyword "ls" o "reg", altrimenti è TK_SLASH.
 *          Ora: '/' seguito da alnum/_ → TK_REGPATH, '/' da solo → TK_REGPATH
 *          solo se è un percorso valido.
 */
...
static struct {
  ...
  TokenKind kind;
    ...
    {"fn", TK_KW_FN},     {"ret", TK_KW_RET},   {"if", TK_KW_IF},
    {"else", TK_KW_ELSE}, {"loop", TK_KW_LOOP}, {"break", TK_KW_BREAK},
    {"cont", TK_KW_CONT}, {"del", TK_KW_DEL},   {"out", TK_KW_OUT},
    {"reg", TK_KW_REG},   {"ls", TK_KW_LS},     {"import", TK_KW_IMPORT},
    {"proc", TK_KW_PROC}, {"and", TK_KW_AND},   {"or", TK_KW_OR},
    {"not", TK_KW_NOT},   {"true", TK_KW_TRUE}, {"false", TK_KW_FALSE},
    {"nil", TK_KW_NIL},   {NULL, TK_EOF}};
...
Token lexer_next(Lexer *lex) {
  ...
  /*
   * Percorso registro: inizia con /
   *
   * FIX: ora gestisce anche "/" da solo come TK_REGPATH (percorso radice).
   * Se '/' è seguito da alnum o '_', è sicuramente un path.
   * Se '/' è da solo, è TK_REGPATH solo se il contesto lo permette
   * (dopo 'ls' o 'reg'). Per semplicità, lo riconosciamo sempre come
   * TK_REGPATH se non c'è un operando prima, o lo trattiamo dopo.
   */
  if (c == '/') {
    size_t i = 0;
    ...
    /* Consuma tutti i caratteri validi per un percorso */
    while (lex->pos < lex->len &&
    ...
    if (i >= 2 && (isalpha((unsigned char)buf[1]) || buf[1] == '_')) {
      /* Percorso tipo /local/x — sicuramente un REGPATH */
      return make_tok(TK_REGPATH, buf, line);
    }
    if (i == 1) {
      /* Solo "/" — è percorso radice o divisione.
       * Lo trattiamo come REGPATH: il parser decide il contesto. */
      return make_tok(TK_REGPATH, buf, line);
    }
    /* Fallback: non è un percorso valido, backtrack a '/' operatore */
    lex->pos = saved_pos + 1;
  ...
  if (isalpha((unsigned char)c) || c == '_') {
    ...
    Token t = make_tok(TK_IDENT, buf, line);
    return t;
  }
  ...
  snprintf(buf, sizeof(buf), "carattere sconosciuto '%c'", c);
