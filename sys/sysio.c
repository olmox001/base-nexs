...
static int nexs_is_fg(void) {
  int pid = (int)getpid();
  if (pid < 100) return 1; // System services always allowed
  
  Value v_fg = reg_get("/sys/kernel/fg_pid");
  if (v_fg.type == TYPE_ERR) return 1; // Not set yet
  
  int fg_pid = (int)val_to_int(&v_fg);
  if (fg_pid == -1) return 1; 
  
  return (pid == fg_pid);
}

static Value bi_set_fg_pid(Value *args, int n) {
  if (n < 1) return val_err(4, "set_fg_pid: requires 1 argument");
  return val_int(reg_set("/sys/kernel/fg_pid", args[0], 0));
}
...
int nexs_open(const char *path, int mode) {
  if (!path) {
    set_errstr("open: NULL path");
    return -1;
  }
  int fd = fd_alloc();
  if (fd < 0) {
    set_errstr("open: fd table full");
    return -1;
  }
  const char *fmode;
  int base = mode & 0x0F;
  ...
  if (!fp) {
    set_errstr("open: cannot open file");
    return -1;
  }
  g_fd_table[fd].fp = fp;
...
int nexs_create(const char *path, int mode, int perm) {
  (void)perm;
  if (!path) {
    set_errstr("create: NULL path");
    return -1;
  }
  int fd = fd_alloc();
  if (fd < 0) {
    set_errstr("create: fd table full");
    return -1;
  }
  const char *fmode;
  int base = mode & 0x0F;
  ...
  FILE *fp = fopen(path, fmode);
  if (!fp) {
    set_errstr("create: cannot create file");
    return -1;
  }
  g_fd_table[fd].fp = fp;
  g_fd_table[fd].emb_src = NULL;
...
int nexs_close(int fd) {
  if (fd < 0 || fd >= NEXS_MAX_FDS) {
    set_errstr("close: invalid fd");
    return -1;
  }
  if (!g_fd_table[fd].in_use) {
    set_errstr("close: fd not open");
    return -1;
  }
...
int nexs_pread(int fd, char *buf, int n, int64_t offset) {
  if (fd < 0 || fd >= NEXS_MAX_FDS || !g_fd_table[fd].in_use) {
    set_errstr("pread: invalid fd");
    return -1;
  }
  if (!buf || n <= 0) {
    set_errstr("pread: invalid params");
    return -1;
  }
  ...
  if (g_fd_table[fd].emb_src) {
    ...
    if (offset < 0)
      g_fd_table[fd].emb_pos += to_read;
    return (int)to_read;
  }

  FILE *fp = g_fd_table[fd].fp;
  if (!fp) {
    set_errstr("pread: NULL fp");
    return -1;
  }
  if (offset >= 0 && fseek(fp, (long)offset, SEEK_SET) != 0) {
    set_errstr("pread: seek failed");
    return -1;
  }
...
int nexs_pwrite(int fd, const char *buf, int n, int64_t offset) {
  if (fd < 0 || fd >= NEXS_MAX_FDS || !g_fd_table[fd].in_use) {
    set_errstr("pwrite: invalid fd");
    return -1;
  }
  if (!buf || n <= 0) {
    set_errstr("pwrite: invalid params");
    return -1;
  }
  FILE *fp = g_fd_table[fd].fp;
  if (!fp) {
    set_errstr("pwrite: NULL fp");
    return -1;
  }
  if (offset >= 0 && fseek(fp, (long)offset, SEEK_SET) != 0) {
    set_errstr("pwrite: seek failed");
    return -1;
  }
...
int64_t nexs_seek(int fd, int64_t offset, int whence) {
  ...
  if (g_fd_table[fd].emb_src) {
    ...
    case 0:
      ...
      break;
    case 1:
      newpos = (int64_t)g_fd_table[fd].emb_pos + offset;
      break;
    case 2:
      newpos = (int64_t)len + offset;
      break;
    default:
      set_errstr("seek: invalid whence");
      return -1;
    }
    if (newpos < 0)
      newpos = 0;
    if (newpos > (int64_t)len)
      newpos = (int64_t)len;
...
int nexs_dup(int oldfd, int newfd) {
  if (oldfd < 0 || oldfd >= NEXS_MAX_FDS || !g_fd_table[oldfd].in_use) {
    set_errstr("dup: invalid oldfd");
    return -1;
  }
  if (newfd < 0)
    newfd = fd_alloc();
  if (newfd < 0 || newfd >= NEXS_MAX_FDS) {
    set_errstr("dup: invalid newfd");
    return -1;
  }
...
int nexs_fd2path(int fd, char *buf, int nbuf) {
  if (fd < 0 || fd >= NEXS_MAX_FDS || !g_fd_table[fd].in_use) {
    set_errstr("fd2path: invalid fd");
    return -1;
  }
  if (!buf || nbuf <= 0) {
    set_errstr("fd2path: invalid buf");
    return -1;
  }
...
int nexs_remove(const char *path) {
  if (!path) {
    set_errstr("remove: NULL path");
    return -1;
  }
  if (remove(path) != 0) {
    set_errstr("remove: failed");
    return -1;
  }
  return 0;
}
...
int nexs_pipe(int fd[2]) {
  if (!fd) {
    set_errstr("pipe: NULL fd");
    return -1;
  }
  int pipefd[2];
  if (pipe(pipefd) != 0) {
    set_errstr("pipe: creation failed");
    return -1;
  }
  int fd0 = fd_alloc();
  if (fd0 < 0) {
...
Value nexs_stat(const char *path) {
  if (!path)
    return val_err(4, "stat: NULL path");
  struct stat sb;
  if (stat(path, &sb) != 0)
    return val_err(4, "stat: file not found");
  char buf[512];
  snprintf(buf, sizeof(buf), "path=%s size=%lld mode=%o mtime=%lld type=%s",
           path, (long long)sb.st_size, (unsigned int)(sb.st_mode & 0777),
           (long long)sb.st_mtime,
           S_ISDIR(sb.st_mode)   ? "dir"
           : S_ISREG(sb.st_mode) ? "file"
                                 : "other");
  return val_str(buf);
}
...
int nexs_chdir(const char *path) {
  if (!path) {
    set_errstr("chdir: NULL path");
    return -1;
  }
  if (chdir(path) != 0) {
    set_errstr("chdir: failed");
    return -1;
  }
  return 0;
}
...
static Value bi_open(Value *args, int n) {
  if (n < 1)
    return val_err(4, "open: requires at least 1 argument");
  ...
  if (fd < 0)
    return val_err(4, "open: failed");
  return val_int(fd);
}

static Value bi_create(Value *args, int n) {
  if (n < 1)
    return val_err(4, "create: requires at least 1 argument");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_write(Value *args, int n) {
  if (n < 2)
    return val_err(4, "write: requires fd and string");
  int fd = (int)val_to_int(&args[0]);
  
  // Focus gating: ignora output video se non in primo piano
  if ((fd == 1 || fd == 2) && !nexs_is_fg()) {
    return val_int(0);
  }
...
static Value bi_seek(Value *args, int n) {
  if (n < 2)
    return val_err(4, "seek: requires fd and offset");
  int fd = (int)val_to_int(&args[0]);
...
static Value bi_dup(Value *args, int n) {
  if (n < 1)
    return val_err(4, "dup: requires at least 1 argument");
...
static Value bi_fd2path(Value *args, int n) {
  if (n < 1)
    return val_err(4, "fd2path: requires 1 argument");
  int fd = (int)val_to_int(&args[0]);
...
static Value bi_remove(Value *args, int n) {
  if (n < 1)
    return val_err(4, "remove: requires 1 argument");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_fstat(Value *args, int n) {
  if (n < 1)
    return val_err(4, "stat: requires 1 argument");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_chdir(Value *args, int n) {
  if (n < 1)
    return val_err(4, "chdir: requires 1 argument");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_mount(Value *args, int n) {
  if (n < 2)
    return val_err(4, "mount: requires src and dst");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_bind(Value *args, int n) {
  if (n < 2)
    return val_err(4, "bind: requires src and dst");
  if (args[0].type != TYPE_STR || !args[0].data)
...
static Value bi_unmount(Value *args, int n) {
  if (n < 1)
    return val_err(4, "unmount: requires at least dst");
  const char *src = NULL, *dst = NULL;
  if (n >= 2) {
    if (args[0].type == TYPE_STR && args[0].data)
      src = (char *)args[0].data;
    if (args[1].type == TYPE_STR && args[1].data)
      dst = (char *)args[1].data;
  } else {
    if (args[0].type == TYPE_STR && args[0].data)
      dst = (char *)args[0].data;
  }
  if (!dst)
    return val_err(4, "unmount: invalid dst");
...
static Value bi_readkey(Value *args, int n) {
  ...
  if (c == 27) { /* ESC */
    ...
    if (c2 == '[') {
      ...
      if (c3 == 'A')
        return val_str("KEY_UP");
      if (c3 == 'B')
        return val_str("KEY_DOWN");
      if (c3 == 'C')
        return val_str("KEY_RIGHT");
      if (c3 == 'D')
        return val_str("KEY_LEFT");

      if (c3 == 'H')
        return val_str("KEY_HOME");
      if (c3 == 'F')
        return val_str("KEY_END");
      ...
      if (c3 >= '1' && c3 <= '6') {
        ...
        if (c3 == '4' || c3 == '8')
          return val_str("KEY_END");
        if (c3 == '3')
          return val_str("KEY_DELETE");
        if (c3 == '5')
          return val_str("KEY_PGUP");
        if (c3 == '6')
          return val_str("KEY_PGDN");
  ...
  if (c == 127 || c == 8)
    return val_str("BACKSPACE");
  if (c == 10 || c == 13)
    return val_str("ENTER");
  if (c == 24)
    return val_str("CTRL_X");
  if (c == 1)
    return val_str("CTRL_A");
  if (c == 5)
    return val_str("CTRL_E");
  if (c == 11)
    return val_str("CTRL_K");
  if (c == 21)
    return val_str("CTRL_U");
  if (c == 23)
    return val_str("CTRL_W");
  if (c == 9)
    return val_str("TAB");
  if (c < 32)
    return val_str(""); /* altri ctrl — ignora */
...
static Value bi_term_at(Value *args, int n) {
  ...
  if (n < 3)
    return val_err(4, "term_at: requires y, x, str");
...
static Value bi_term_cursor_move(Value *args, int n) {
  if (!nexs_is_fg()) return val_nil();
  if (n < 2)
    return val_err(4, "term_cursor_move: requires y, x");
  int y = (int)val_to_int(&args[0]);
...
static Value bi_term_cursor_show(Value *args, int n) {
  if (!nexs_is_fg()) return val_nil();
  if (n < 1)
    return val_err(4, "term_cursor_show: requires bool");
...
void sysio_register_builtins(void) {
  ...
  fn_register_builtin_sig("close", bi_close, SIG("close(fd int)") "nil");
  fn_register_builtin_sig("read", bi_read, SIG("read(fd int, n int)") "str");
  ...
  fn_register_builtin_sig("fd2path", bi_fd2path, SIG("fd2path(fd int)") "str");
  fn_register_builtin_sig("remove", bi_remove, SIG("remove(path str)") "int");
  fn_register_builtin_sig("pipe", bi_pipe, SIG("pipe()") "arr [rfd wfd]");
  fn_register_builtin_sig("fstat", bi_fstat, SIG("fstat(path str)") "str");
  fn_register_builtin_sig("chdir", bi_chdir, SIG("chdir(path str)") "int");
  ...
  fn_register_builtin_sig("rawon", bi_rawon, SIG("rawon()") "int");
  fn_register_builtin_sig("rawoff", bi_rawoff, SIG("rawoff()") "int");
  fn_register_builtin_sig("readbyte", bi_readbyte, SIG("readbyte()") "int");
  fn_register_builtin_sig("chr", bi_chr, SIG("chr(codepoint int)") "str");
  fn_register_builtin_sig("set_layout", bi_set_layout,
                          SIG("set_layout(name str)") "int");
  fn_register_builtin_sig("set_fg_pid", bi_set_fg_pid, SIG("set_fg_pid(pid)") "int");
  fn_register_builtin_sig("readkey", bi_readkey, SIG("readkey()") "str");

  fn_register_builtin_sig("_readkey", bi_readkey, SIG("_readkey()") "str");
  ...
  fn_register_builtin_sig("_term_cls", bi_term_cls, SIG("_term_cls()") "nil");
  fn_register_builtin_sig("_term_cursor_move", bi_term_cursor_move,
                          SIG("_term_cursor_move(y int, x int)") "nil");
  fn_register_builtin_sig("_term_erase_eol", bi_term_erase_eol,
                          SIG("_term_erase_eol()") "nil");
  ...
  /* Store actual fn_table indices in /sys/<name> for val_print and eval
   * resolution */
  {
    static const struct {
      const char *name;
      BuiltinFn fn;
        ...
        {"rawon", bi_rawon},
        {"rawoff", bi_rawoff},
        {"readbyte", bi_readbyte},
        {"chr", bi_chr},
        {"set_layout", bi_set_layout},
        {"readkey", bi_readkey},
        {"readkey_nb", bi_readkey_nb},
        {"_term_at", bi_term_at},
        {"_term_cls", bi_term_cls},
        {"_term_cursor_move", bi_term_cursor_move},
        {"_term_erase_eol", bi_term_erase_eol},
        {"_term_cursor_show", bi_term_cursor_show},
        {"_term_flush", bi_term_flush},
