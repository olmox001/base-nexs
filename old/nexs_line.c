...
static int utf8_col_width(uint32_t cp) {
    ...
    if (cp >= 0x1100 && cp <= 0x9FFF) return 2;
    if (cp >= 0xF900 && cp <= 0xFFFF) return 2;
...
static uint32_t utf8_decode(const char *buf, int len, int *pos) {
    ...
    if (*pos + seqlen > len) { (*pos)++; return c; }
...
NxKey nexs_key_read(void) {
    ...
    int c = read_byte();
    if (c == -1) { k.code = NXK_EOF; return k; }

    /* ── Escape sequences ──────────────────────────────────── */
    if (c == 0x1B) {
        int c2 = read_byte_timeout();
        if (c2 == -1) { k.code = NXK_ESC; return k; }

        if (c2 == '[') {
            /* CSI sequence: ESC [ ... */
            int c3 = read_byte_timeout();
            if (c3 == -1) { k.code = NXK_ESC; return k; }

            /* ESC [ A/B/C/D — bare arrows */
            if (c3 == 'A') { k.code = NXK_UP;    return k; }
            if (c3 == 'B') { k.code = NXK_DOWN;  return k; }
            if (c3 == 'C') { k.code = NXK_RIGHT; return k; }
            if (c3 == 'D') { k.code = NXK_LEFT;  return k; }
            if (c3 == 'H') { k.code = NXK_HOME;  return k; }
            if (c3 == 'F') { k.code = NXK_END;   return k; }
            ...
            if (c3 == '1') {
                ...
                if (c4 == ';') {
                    ...
                    if (mod == '2') {
                        if (dir == 'D') { k.code = NXK_SHIFT_LEFT;  return k; }
                        if (dir == 'C') { k.code = NXK_SHIFT_RIGHT; return k; }
                        if (dir == 'A') { k.code = NXK_UP;          return k; }
                        if (dir == 'B') { k.code = NXK_DOWN;        return k; }
                    }
                    if (mod == '5') {
                        if (dir == 'D') { k.code = NXK_CTRL_LEFT;  return k; }
                        if (dir == 'C') { k.code = NXK_CTRL_RIGHT; return k; }
                    }
                } else if (c4 == '~') {
                    k.code = NXK_HOME; return k; /* ESC [ 1 ~ */
                }
            ...
            if (c3 >= '2' && c3 <= '6') {
                int c4 = read_byte_timeout();
                if (c4 == '~') {
                    if (c3 == '3') { k.code = NXK_DELETE; return k; }
                    if (c3 == '4') { k.code = NXK_END;    return k; }
                    if (c3 == '5') { k.code = NXK_PGUP;   return k; }
                    if (c3 == '6') { k.code = NXK_PGDN;   return k; }
                }
            }
    ...
    /* ── Control characters ────────────────────────────────── */
    switch (c) {
        case '\r': case '\n': k.code = NXK_ENTER;     return k;
        case 0x7F:            k.code = NXK_BACKSPACE;  return k;
        case 0x08:            k.code = NXK_BACKSPACE;  return k;
        case 0x09:            k.code = NXK_TAB;        return k;
        case 0x01:            k.code = NXK_CTRL_A;     return k;
        case 0x02:            k.code = NXK_CTRL_B;     return k;
        case 0x03:            k.code = NXK_CTRL_C;     return k;
        case 0x04:            k.code = NXK_CTRL_D;     return k;
        case 0x05:            k.code = NXK_CTRL_E;     return k;
        case 0x06:            k.code = NXK_CTRL_F;     return k;
        case 0x0B:            k.code = NXK_CTRL_K;     return k;
        case 0x0C:            k.code = NXK_CTRL_L;     return k;
        case 0x0E:            k.code = NXK_CTRL_N;     return k;
        case 0x10:            k.code = NXK_CTRL_P;     return k;
        case 0x12:            k.code = NXK_CTRL_R;     return k;
        case 0x14:            k.code = NXK_CTRL_T;     return k;
        case 0x15:            k.code = NXK_CTRL_U;     return k;
        case 0x17:            k.code = NXK_CTRL_W;     return k;
        case 0x19:            k.code = NXK_CTRL_Y;     return k;
    }
...
const char *nexs_key_name(NxKeyCode kc) {
    switch (kc) {
        case NXK_NONE:        return "NONE";
        case NXK_CHAR:        return "CHAR";
        case NXK_ENTER:       return "ENTER";
        case NXK_BACKSPACE:   return "BACKSPACE";
        case NXK_DELETE:      return "DELETE";
        case NXK_LEFT:        return "LEFT";
        case NXK_RIGHT:       return "RIGHT";
        case NXK_UP:          return "UP";
        case NXK_DOWN:        return "DOWN";
        case NXK_HOME:        return "HOME";
        case NXK_END:         return "END";
        case NXK_PGUP:        return "PGUP";
        case NXK_PGDN:        return "PGDN";
        case NXK_SHIFT_LEFT:  return "SHIFT_LEFT";
        case NXK_SHIFT_RIGHT: return "SHIFT_RIGHT";
        case NXK_CTRL_LEFT:   return "CTRL_LEFT";
        case NXK_CTRL_RIGHT:  return "CTRL_RIGHT";
        case NXK_CTRL_A:      return "CTRL_A";
        case NXK_CTRL_B:      return "CTRL_B";
        case NXK_CTRL_C:      return "CTRL_C";
        case NXK_CTRL_D:      return "CTRL_D";
        case NXK_CTRL_E:      return "CTRL_E";
        case NXK_CTRL_F:      return "CTRL_F";
        case NXK_CTRL_K:      return "CTRL_K";
        case NXK_CTRL_L:      return "CTRL_L";
        case NXK_CTRL_N:      return "CTRL_N";
        case NXK_CTRL_P:      return "CTRL_P";
        case NXK_CTRL_R:      return "CTRL_R";
        case NXK_CTRL_T:      return "CTRL_T";
        case NXK_CTRL_U:      return "CTRL_U";
        case NXK_CTRL_W:      return "CTRL_W";
        case NXK_CTRL_Y:      return "CTRL_Y";
        case NXK_TAB:         return "TAB";
        case NXK_ESC:         return "ESC";
        case NXK_EOF:         return "EOF";
...
void nexs_line_render(NxLineEditor *le) {
    ...
    write_str("\r");
    /* prompt */
    if (le->prompt) write_str(le->prompt);
    /* buffer content */
    ...
    if (cols_after > 0) cursor_left(cols_after);
}
...
void nexs_line_add_history(NxLineEditor *le, const char *line) {
    ...
    if (le->hist_count > 0) {
        ...
        if (strcmp(le->history[last], line) == 0) return;
    }
...
static void history_nav(NxLineEditor *le, int dir) {
    ...
    if (new_nav >= le->hist_count) return; /* can't go further forward */
...
static void insert_chars(NxLineEditor *le, const char *s, int slen) {
    if (le->len + slen >= NEXS_LINE_BUFSIZE) return;
...
static void delete_char_before(NxLineEditor *le) {
    ...
    memmove(le->buf + prev, le->buf + le->pos,
            (size_t)(le->len - le->pos));
    le->len -= dlen;
    le->pos = prev;
    le->buf[le->len] = '\0';
...
int nexs_line_read(NxLineEditor *le, const char *prompt,
                    char *out, int outsz) {
    /* Non-tty fallback */
