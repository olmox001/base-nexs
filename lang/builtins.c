...
Value builtin_vfs_resolve(Value *args, int n) {
  if (n < 2)
    return val_err(4, "vfs_resolve: path, cwd");
  ...
  if (path[0] == '\0')
    return val_str(cwd);
  if (path[0] == '/')
    return val_str(path); /* Keep absolute as is */
...
Value builtin_vfs_cd(Value *args, int n) {
  ...
  if (path[0] == '/') {
    ...
    if (strcmp(cwd, "/") == 0)
      ...
      snprintf(target, sizeof(target), "%s/%s", cwd, path);
  }
...
Value builtin_vfs_ls(Value *args, int n) {
  const char *dir =
      (n >= 1 && args[0].type == TYPE_STR) ? (char *)args[0].data : "";
  FILE *out =
      (nexs_g_eval_ctx && nexs_g_eval_ctx->out) ? nexs_g_eval_ctx->out : stdout;

  char clean_dir[REG_PATH_MAX];
  if (dir[0] == '\0') {
    Value v = reg_get("/proc/1/vfs_cwd");
    if (v.type == TYPE_STR && v.data)
      strncpy(clean_dir, (char *)v.data, REG_PATH_MAX - 1);
    else
      strcpy(clean_dir, "/");
    val_free(&v);
  } else {
...
Value builtin_vfs_read(Value *args, int n) {
  if (n < 1 || args[0].type != TYPE_STR)
    return val_err(4, "vfs_read: path");
  const char *path = (char *)args[0].data;
...
Value builtin_vfs_write(Value *args, int n) {
  if (n < 2 || args[0].type != TYPE_STR)
    return val_err(4, "vfs_write: path, data");
  const char *path = (char *)args[0].data;
  ...
  for (int i = 0; i < count; i++) {
    ...
    Value v_name = reg_get(idx_path);
    if (v_name.type == TYPE_STR && v_name.data &&
        strcmp((char *)v_name.data, path) == 0) {
      found = 1;
      val_free(&v_name);
      break;
    }
...
/* vfs_exists(path) -> bool */
Value builtin_vfs_exists(Value *args, int n) {
  if (n < 1 || args[0].type != TYPE_STR)
    return val_err(4, "vfs_exists: path");
  const char *path = (char *)args[0].data;
  char target[REG_PATH_MAX];
  snprintf(target, sizeof(target), "/sys/vfs/files/%s", path);
  Value v = reg_get(target);
  int exists = (v.type != TYPE_ERR);
  val_free(&v);
  return val_bool(exists);
}

/* vfs_normalize_path(path) -> str (stub for now, just cleans slashes) */
Value builtin_vfs_normalize_path(Value *args, int n) {
  if (n < 1 || args[0].type != TYPE_STR)
    return val_err(4, "vfs_normalize_path: path");
  const char *path = (char *)args[0].data;
  char buf[REG_PATH_MAX];
  strncpy(buf, path, REG_PATH_MAX - 1);
  buf[REG_PATH_MAX - 1] = '\0';
  /* Very simple normalization: remove trailing slash if not root */
  size_t len = strlen(buf);
  if (len > 1 && buf[len - 1] == '/')
    buf[len - 1] = '\0';
  return val_str(buf);
...
Value builtin_vfs_mkdir(Value *args, int n) {
  if (n < 1 || args[0].type != TYPE_STR)
    return val_err(4, "vfs_mkdir: path");
  const char *path = (char *)args[0].data;
  ...
  for (int i = 0; i < count; i++) {
    ...
    Value v = reg_get(chk);
    int dup =
        (v.type == TYPE_STR && v.data && strcmp((char *)v.data, dpath) == 0);
    val_free(&v);
    if (dup)
      return val_nil();
  }
...
void builtins_register_all(void) {
  ...
  register_builtin_sig("sys_bundled", builtin_sys_bundled,
                       SIG("sys_bundled()") "arr");
  register_builtin_sig("ls", builtin_vfs_ls, SIG("ls(dir str)") "nil");
  register_builtin_sig("cd", builtin_vfs_cd, SIG("cd(path str)") "nil");
  register_builtin_sig("pwd", builtin_vfs_pwd, SIG("pwd()") "str");
  register_builtin_sig("vfs_resolve", builtin_vfs_resolve,
                       SIG("vfs_resolve(path str, cwd str)") "str");
  register_builtin_sig("vfs_ls", builtin_vfs_ls, SIG("vfs_ls(dir str)") "nil");
  register_builtin_sig("vfs_cd", builtin_vfs_cd, SIG("vfs_cd(path str)") "nil");
  register_builtin_sig("vfs_pwd", builtin_vfs_pwd, SIG("vfs_pwd()") "str");
  ...
  register_builtin_sig("vfs_rm", builtin_vfs_rm, SIG("vfs_rm(path str)") "nil");
  register_builtin_sig("vfs_mkdir", builtin_vfs_mkdir,
                       SIG("vfs_mkdir(path str)") "nil");
  register_builtin_sig("vfs_exists", builtin_vfs_exists,
                       SIG("vfs_exists(path str)") "bool");
  register_builtin_sig("vfs_normalize_path", builtin_vfs_normalize_path,
                       SIG("vfs_normalize_path(path str)") "str");
