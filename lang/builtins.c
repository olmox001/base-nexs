...
/* Weak definition so it links even when not provided by codegen.c */
__attribute__((weak)) const NexsEmbedDep nexs_embed_deps_table[] = { {NULL, NULL} };
...
static Value builtin_sys_bundled(Value *args, int n) {
  ...
  for (int i = 0; nexs_embed_deps_table[i].path; i++) {
    Value tmp = val_str(nexs_embed_deps_table[i].path);
    arr_set(arr, idx++, tmp);
    val_free(&tmp);
  }
#endif
