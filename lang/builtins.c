...
static Value builtin_sys_embedded_read(Value *args, int n) {
  ...
  __attribute__((weak)) extern const char *nexs_embedded_lookup(const char *path);
  if (!nexs_embedded_lookup) return val_err(4, "Embedded lookup not available in this build");
  const char *src = nexs_embedded_lookup(path);
  if (!src) return val_err(4, "File not found in embedded table");
  return val_str(src);
}
...
static Value builtin_sys_bundled(Value *args, int n) {
  ...
  __attribute__((weak)) extern const NexsEmbedDep nexs_embed_deps_table[];
  
  /* The table is generated by codegen.c in standalone builds */
  if (nexs_embed_deps_table) {
    for (int i = 0; nexs_embed_deps_table[i].path; i++) {
      arr_set(arr, idx++, val_str(nexs_embed_deps_table[i].path));
    }
  }
#endif
  Value v;
